      ...
      <div className="max-w-5xl w-full mx-auto p-8 print:px-0 print:py-4 mt-8 print:mt-0">
        ...
        <div className="mb-8 print:mb-4 border-b-4 print:border-b-2 border-slate-900 pb-4 print:pb-2">
          <div className="flex items-center gap-3 print:gap-2">
            ...
            <h1 style={{ pageBreakAfter: "avoid", breakAfter: "avoid" }} className="text-3xl print:text-2xl font-black text-slate-900 tracking-tight leading-tight">
        ...
        <div className="mb-12 print:mb-8 print:break-after-page">
          <h2 style={{ pageBreakAfter: "avoid", breakAfter: "avoid" }} className="text-2xl font-black text-slate-900 mb-4 pb-2 border-b border-slate-300 flex items-center gap-3">
        ...
        <div className="mb-12 print:mb-8 print:break-after-page">
          ...
          <div className="prose prose-slate max-w-none text-[17px] leading-8 text-slate-800 [&>p]:mb-5 [&_ul]:mb-6 [&_li]:mb-2">
            <p style={{ pageBreakAfter: "avoid", breakAfter: "avoid" }}><strong>Amazon EKS</strong> is highly-managed Kubernetes. In EKS, AWS fully obscures and manages the Kubernetes "Control Plane" (the master nodes, API Server, etcd state database). Your only responsibility is securing and scaling the "Data Plane" (the EC2 Worker Nodes executing your Docker containers).</p>
            
            <h3 style={{ pageBreakAfter: "avoid", breakAfter: "avoid" }} className="text-xl font-bold text-slate-900 mt-8 mb-4">Mastering Autoscaling in EKS</h3>
            ...
            <h3 style={{ pageBreakAfter: "avoid", breakAfter: "avoid" }} className="text-xl font-bold text-slate-900 mt-8 mb-4">EKS Security: IAM Roles for Service Accounts (IRSA)</h3>
            <p style={{ pageBreakAfter: "avoid", breakAfter: "avoid" }}><strong>Crucial Concept:</strong> You never assign an AWS IAM Role to an entire EC2 Worker Node. If you do, <em>every</em> pod on that node inherits those permissions (violating Least Privilege). Instead, establish an OIDC Identity Provider on the EKS cluster, and bind specific AWS IAM Roles directly to individual Kubernetes <code className="bg-slate-100 border border-slate-300 text-slate-900 px-2 py-0.5 rounded text-sm">ServiceAccounts</code>. This ensures your Python OCR pod can read S3, but your Node.js pod cannot.</p>
          </div>
        </div>
        ...
        <div className="mb-12 print:mb-8 print:break-after-page">
          ...
          <div className="prose prose-slate max-w-none text-[17px] leading-8 text-slate-800 [&>p]:mb-5 [&_ul]:mb-6 [&_li]:mb-2">
            <p style={{ pageBreakAfter: "avoid", breakAfter: "avoid" }}><strong>AWS Lambda</strong> is the ultimate manifestation of serverless compute. It scales from zero to 10,000 parallel executions in seconds with absolutely zero container orchestration overhead required. However, Lambda functions are fundamentally <em>stateless</em> and strictly limited to a 15-minute maximum runtime execution timeout.</p>

            <h3 style={{ pageBreakAfter: "avoid", breakAfter: "avoid" }} className="text-xl font-bold text-slate-900 mt-6 mb-3">Orchestration vs Choreography</h3>
            <p style={{ pageBreakAfter: "avoid", breakAfter: "avoid" }}>Microservices often require complex, multi-step transaction lifecycles (e.g., Extract &rarr; Validate &rarr; Sync &rarr; Notify). Managing state sequentially across dozens of Lambdas is an anti-pattern. The solution is <strong>AWS Step Functions</strong>—a visual state machine that governs exact execution flow, handling complex retry logic, error-catching, and parallel branching seamlessly outside of the code block.</p>
            
            <h3 style={{ pageBreakAfter: "avoid", breakAfter: "avoid" }} className="text-xl font-bold text-slate-900 mt-8 mb-4">Event-Driven Summary Pipeline</h3>
            <p style={{ pageBreakAfter: "avoid", breakAfter: "avoid" }}>If you need an API, wrap Lambda tightly behind an <strong>AWS API Gateway</strong>. If you are handling millions of async events, utilize <strong>Amazon SQS</strong> (Queues) or <strong>MSK/Kafka</strong> (Message Streaming) to fan-out events to your Lambda consumers.</p>
          </div>
        </div>
