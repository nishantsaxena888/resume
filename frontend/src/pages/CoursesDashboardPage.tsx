    ...
    const initialize = async () => {
      try {
        if (prepId) {
           ...
           // Global Hub - Load all courses without a Prep association
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col p-10 font-sans w-full">
      <div className="max-w-4xl mx-auto w-full flex flex-col gap-8">
        ...
        <header className="flex items-center justify-between border-b border-slate-200 pb-6">
           <div>
             <h1 className="text-2xl font-black text-slate-900 tracking-tight">{prepId ? 'Curriculum Manager' : 'Global Course Hub'}</h1>
             <p className="text-sm font-medium text-slate-500">{prepId ? 'Bind independent study modules into your Active Preparation context.' : 'Select a course below to drill into the specific syllabus and interactive modules.'}</p>
           </div>
           {prepId && <button 
             onClick={handleSave} 
             disabled={isSaving}
               ...
               allCourses.map((course) => {
                 ...
                 return (
                   <div 
                     ...
                     className={`cursor-pointer transition-all duration-200 p-5 rounded-xl border flex flex-col gap-3 group ${isSelected && prepId ? 'bg-indigo-50 border-indigo-400 shadow-inner' : 'bg-white border-slate-200 hover:border-indigo-300 hover:shadow-sm'}`}
                   >
                     <div className="flex items-start justify-between">
                       <div className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors ${isSelected && prepId ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 text-slate-400 group-hover:bg-indigo-100 group-hover:text-indigo-600'}`}>
                         <BookOpen className="w-5 h-5" />
                       </div>
                       {prepId && <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${isSelected ? 'border-indigo-600 bg-indigo-600' : 'border-slate-300 bg-white'}`}>
