...
from models import User, Course, CourseModule, CourseWidget, Resume, JobDescription, Preparation, PreparationNote, AIPrompt
from schemas import (
...
def generate_ai_notes(
    ...
    ai_prompt_obj = db.query(AIPrompt).filter(AIPrompt.key == req.prompt_key).first()
    if not ai_prompt_obj:
        raise HTTPException(status_code=404, detail=f"The specialized system prompt key '{req.prompt_key}' could not be located in the resilient postgres database.")
        
    client = genai.Client(api_key=api_key)
    prompt = ai_prompt_obj.prompt_text + "\n\nTranscript JSON map:\n" + transcript_text
...
def get_prompts(db: Session = Depends(get_db)):
    """Extract all resilient Database Prompts mapping AI configuration states"""
    prompts = db.query(AIPrompt).order_by(AIPrompt.created_at.desc()).all()
    return [{"id": p.id, "key": p.key, "title": p.title, "prompt_text": p.prompt_text} for p in prompts]

@router.post("/prompts")
def create_prompt(payload: AIPromptCreate, db: Session = Depends(get_db)):
    """Hook to autonomously engineer new Prompt maps or overwrite keys natively"""
    existing = db.query(AIPrompt).filter(AIPrompt.key == payload.key).first()
    if existing:
        existing.title = payload.title
        existing.prompt_text = payload.prompt_text
        db.commit()
        return {"success": True, "message": "Updated immutable AI Prompt configuration explicitly."}
    
    new_prompt = AIPrompt(key=payload.key, title=payload.title, prompt_text=payload.prompt_text)
    db.add(new_prompt)
    db.commit()
    return {"success": True, "message": "Deployed structural AI Prompt instance."}
