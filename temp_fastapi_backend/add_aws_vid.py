...
try:
    ...
    # Create the New Module natively underneath the targeted AWS Course
    mod_aws = CourseModule(id=max_module_id + 1, course_id=c_aws.id, title="AWS Deep Dive & Advanced Architecture")
    db.add(mod_aws)
    db.commit()
    db.refresh(mod_aws)
    ...
    w_aws = CourseWidget(
        ...
        widget_type="youtube",
        payload={
            "title": "AWS Serverless & Advanced Services Tutorial",
            "url": "https://www.youtube.com/watch?v=3WZzmiAkYBQ"
        }
    )
    db.add(w_aws)
    db.commit()
